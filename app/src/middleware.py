    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            if "multipart/form-data" in request.headers.get("content-type", ""):
                ...
                logging.info(f"Imagem recebida: {image.filename if image else 'Nenhuma'}")
        ...
        if image and audio and "/analyze-image-audio-query" in request.url.path:
            ...
            try:
                ...
                logging.info(f"Áudio salvo em: {audio_path} (tamanho: {len(audio_content)} bytes)")

                # Salvar a imagem
                image_content = await image.read()
                image_extension = os.path.splitext(image.filename)[-1] or ".jpg"  # Preservar a extensão original
                image_path = os.path.join(folder_path, f"image{image_extension}")
                with open(image_path, "wb") as f:
                    f.write(image_content)
                logging.info(f"Imagem salva em: {image_path} (tamanho: {len(image_content)} bytes)")
                ...
                logging.info(f"Resposta salva em: {response_path} (tamanho: {len(response_body)} bytes)")
